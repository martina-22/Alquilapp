...
export const VerReserva = () => {
  ...
  useEffect(() => {
    const fetchReservas = async () => {
      try {
        const res = await fetch(`http://localhost:5000/reservas/ver/${usuarioId}`)
        const data = await res.json()

        const reservasConVehiculo = await Promise.all(
          data.map(async (reserva: any) => {
            try {
              const vehiculoRes = await fetch(`http://localhost:5000/vehiculos/${reserva.vehiculo_id}`)
              ...
              return { ...reserva, nombre_vehiculo: 'Vehículo desconocido' }
            }
          })
        )
  ...
  const confirmarCancelacion = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/reservas/cancelar/${reservaId}`, {
        method: "PUT"
      })
      if (res.ok) {
  ...
  return (
    <Box
      ...
      sx={{ background: '#ffffff', color: 'white', p: 4, gap: 3 }}
    >
      <Box width="100%" maxWidth={800} display="flex" alignItems="center">
        <IconButton onClick={handleVolver} sx={{ color: '#45e5ff', mr: 2 }}>
          <ArrowBackIcon />
        </IconButton>
        <Typography variant="h4" fontWeight={600} color="#000">Mis Reservas</Typography>
      </Box>
          ...
          >
            <CardContent>
              <Box display="flex" justifyContent="space-between">
                <Typography variant="h6" sx={{ color: '#45e5ff' }} gutterBottom>
                  Reserva #{idx + 1}
                </Typography>
                <IconButton onClick={(e) => handleMenuClick(e, idx)} sx={{ color: '#45e5ff' }}>
                  <MoreVertIcon />
                </IconButton>
                <Menu
              ...
              <Typography>Vehículo: {reserva.nombre_vehiculo}</Typography>
              <Typography>Fecha: {reserva.fecha_inicio} a {reserva.fecha_fin}</Typography>
              <Typography>Estado: {reserva.estado_nombre}</Typography>
            </CardContent>
          </Card>
        ))
      ) : (
      ...
      <Button
        ...
        sx={{
          mt: 4,
          backgroundColor: "#45e5ff",
          color: "#000",
          fontWeight: "bold",
          "&:hover": { backgroundColor: "#30BBCB" }
        }}
      >
        Volver al Menú
      </Button>
      ...
      <Dialog open={openDetalle} onClose={handleCerrarDetalle}>
        ...
        <DialogContent>
          {detalleReserva && (
            <>
              <Typography>Vehículo: {detalleReserva.nombre_vehiculo}</Typography>
              <Typography>Fecha inicio: {detalleReserva.fecha_inicio}</Typography>
              <Typography>Fecha fin: {detalleReserva.fecha_fin}</Typography>
              <Typography>Hora de retiro: {detalleReserva.hora_retiro || 'No especificada'}</Typography>
              <Typography>Hora de devolución: {detalleReserva.hora_devolucion || 'No especificada'}</Typography>
              <Typography>Estado: {detalleReserva.estado_nombre}</Typography>
              <Typography>Pagada: {detalleReserva.pagada ? 'Sí' : 'No'}</Typography>
              <Typography>Monto total: ${detalleReserva.monto_total}</Typography>
            </>
          )}
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCerrarDetalle} color="primary">
            Cerrar
          </Button>
        </DialogActions>
      </Dialog>
