...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSucursales = async () => {
      try {
        const response = await fetch('http://localhost:5000/sucursales/all'); // url_prefix='/sucursales'
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data: Sucursal[] = await response.json();
        setSucursalesData(data); // Guarda los datos en el estado
      } catch (error: any) {
        console.error("Error al cargar sucursales:", error);
        setErrorSucursales(`No se pudieron cargar las sucursales: ${error.message}`);
      } finally {
        setLoadingSucursales(false); // Indica que la carga ha terminado
      }
    };
      ...
      <Box sx={{ bgcolor: 'background.default', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <AppBar position="static" color="inherit" elevation={1}>
          <Toolbar sx={{ justifyContent: 'space-between', px: { xs: 2, md: 4 }, py: { xs: 1, md: 2 } }}>
            <Box component="img" src="/icons/logo.png" alt="AlquilApp Car" sx={{ height: { xs: 100, md: 120 } }} />
            <Box
              sx={{
                display: 'flex',
                flexDirection: 'column',
                alignItems: 'flex-end',
                gap: 0.8, // en MUI: 1 = 8px
              }}
            >
        ...
        <Container maxWidth="lg" sx={{ flexGrow: 1, py: { xs: 2, md: 6 } }}>
          {/* Reservá tu auto */}
          <Box sx={{ my: { xs: 4, md: 6 } }}>
            <Typography variant="h4" component="h2" align="center" sx={{ mb: { xs: 3, md: 4 }, fontWeight: 'bold', color: 'text.primary' }}>
              Reservá tu auto
            </Typography>
          ...
          <Box sx={{ my: { xs: 4, md: 6 }, display: 'flex', justifyContent: 'center' }}>
            <Button variant="contained" color="primary" sx={{ width: { xs: '100%', sm: 256 }, py: 1.5, fontWeight: 'bold' }}>
              Buscar
            </Button>
          </Box>
        ...
        <Container maxWidth="lg" sx={{ flexGrow: 1, py: { xs: 2, md: 4 } }}>
          ...
          <Box sx={{ my: { xs: 4, md: 6 } }}>
            <Typography variant="h5" component="h3" align="center" sx={{ mb: { xs: 3, md: 4 }, fontWeight: 'bold', color: 'text.primary' }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: { xs: 1, md: 1.5 }, mt: 2 }}>
              {categories.map((cat, idx) => (
                <Button
                  ...
                  variant="outlined"
                  color="primary"
                  sx={{ borderColor: 'primary.main', color: 'text.primary', '&:hover': { bgcolor: 'secondary.main' } }}
        ...
        >
          ...
          <Box sx={{
            ...
            <Button
              variant="contained"
              color="secondary"
              sx={{ px: { xs: 2, md: 4 }, py: { xs: 1.5, md: 2 }, fontWeight: 'bold', minWidth: { xs: 'auto', md: 150 } }}
            >
              Enviar
            </Button>
          </Box>
